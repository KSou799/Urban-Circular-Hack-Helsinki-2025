...
#
# WHAT THIS DOES
#   Simulates a city of ~25 flexible electricity loads (EVs, heat pumps,
...
#
# OPTIMIZER
#   - Minimize peak load (simulated annealing with multi-restart)
#
...
ASSETS = [
    {"owner": "City EV fleet",        "appliance": "Fast chargers",         "power_kW": 80.0, "duration_h": 4, "start_hour": 17, "end_hour": 24, "variable": True,  "enabled": True, "flex_factor": 0.95},
    {"owner": "Office block A",       "appliance": "HVAC pre-cooling",      "power_kW": 30.0, "duration_h": 3, "start_hour": 14, "end_hour": 20, "variable": True,  "enabled": True, "flex_factor": 0.90},
    ...
    {"owner": "Residential block",    "appliance": "EV chargers",           "power_kW": 40.0, "duration_h": 6, "start_hour": 18, "end_hour": 24, "variable": True,  "enabled": True, "flex_factor": 0.90},
    ...
    {"owner": "Metro station",        "appliance": "Ventilation",           "power_kW": 35.0, "duration_h": 4, "start_hour": 10, "end_hour": 18, "variable": True,  "enabled": True, "flex_factor": 0.85},
    ...
    {"owner": "Downtown chargers",    "appliance": "Public EV charging",    "power_kW": 50.0, "duration_h": 5, "start_hour": 17, "end_hour": 24, "variable": True,  "enabled": True, "flex_factor": 0.85},
    ...
    {"owner": "Logistics warehouse",  "appliance": "Pre-chill cooling",     "power_kW": 22.0, "duration_h": 4, "start_hour": 12, "end_hour": 20, "variable": True,  "enabled": True, "flex_factor": 0.85},
    ...
    {"owner": "Central library",      "appliance": "HVAC",                  "power_kW": 10.0, "duration_h": 4, "start_hour": 8,  "end_hour": 18, "variable": True,  "enabled": True, "flex_factor": 0.85},
    {"owner": "Small shops",          "appliance": "Refrigeration",         "power_kW": 18.0, "duration_h": 6, "start_hour": 8,  "end_hour": 22, "variable": True,  "enabled": True, "flex_factor": 0.65},
    {"owner": "Residential solar",    "appliance": "Battery charging",      "power_kW": 15.0, "duration_h": 5, "start_hour": 10, "end_hour": 20, "variable": True,  "enabled": True, "flex_factor": 1.0},
    {"owner": "Electric bus depot",   "appliance": "Overnight charging",    "power_kW": 90.0, "duration_h": 6, "start_hour": 22, "end_hour": 6,  "variable": True,  "enabled": True, "flex_factor": 0.95},
]
...
ttk.Label(stats_frame, text="CO2 saving: ").grid(row=6, column=0, sticky="w")
...
input_frame.grid(row=1, column=0, sticky="new", pady=(10, 0))
...
priority_frame.grid(row=2, column=0, sticky="new", pady=(10, 0))
...
flex_frame.grid(row=3, column=0, sticky="new", pady=(10, 0))
...
def run_sa_optimization(objective_fn, num_restarts=3, max_iter=120,
                        temp_start=25.0, cooling=0.95, step=20):
    """
    Multi-restart simulated annealing over (flex%, priority%).

    Tracks the current state AND the best-seen state separately, so
    SA can accept temporarily-worse moves without losing the optimum
    (a bug in the previous single-state implementation).

    objective_fn() must return a scalar to minimize and must be
    evaluated AFTER recalculate_schedules() has been called.

    Returns (best_flex, best_priority, best_obj).
    """
    best_overall = None

    for _ in range(num_restarts):
        cur_flex = random.randint(0, 100)
        cur_priority = random.randint(0, 100)
        flex_var.set(cur_flex)
        priority_var.set(cur_priority)
        recalculate_schedules()
        cur_obj = objective_fn()

        best_flex, best_priority, best_obj = cur_flex, cur_priority, cur_obj

        temperature = temp_start
        for _ in range(max_iter):
            if temperature <= 0.05:
                break
            new_flex = clamp(cur_flex + random.randint(-step, step), 0, 100)
            new_priority = clamp(cur_priority + random.randint(-step, step), 0, 100)

            flex_var.set(new_flex)
            priority_var.set(new_priority)
            recalculate_schedules()
            new_obj = objective_fn()
            delta = new_obj - cur_obj

            if delta < 0 or random.random() < math.exp(-delta / (temperature + 0.1)):
                cur_flex, cur_priority, cur_obj = new_flex, new_priority, new_obj
                if cur_obj < best_obj:
                    best_flex, best_priority, best_obj = cur_flex, cur_priority, cur_obj

            temperature *= cooling

        if best_overall is None or best_obj < best_overall[2]:
            best_overall = (best_flex, best_priority, best_obj)
...
def find_optimal_settings_fast():
    """Minimize peak load."""
    best_flex, best_priority, best_peak = run_sa_optimization(
        objective_fn=lambda: max(after_load)
    )
    apply_optimization_result(best_flex, best_priority)
    messagebox.showinfo(
        "Peak optimization",
        f"Best peak: {best_peak:.1f} units\n"
        f"Flex: {best_flex}% | Grid/Price: {100-best_priority}%/{best_priority}%"
    )


ttk.Button(flex_frame, text="[*] Optimize peak (FAST)",
           command=find_optimal_settings_fast)\
...
def get_y_scaling_params(usable_height):
...
def draw_single_graph(canvas, load, title):
    ...
    max_tick = get_y_scaling_params(usable_height)
...
def draw_combined_graph():
    ...
    canvas.create_line(*to_pts(before_load), fill="blue", width=2)
    canvas.create_line(*to_pts(after_load), fill="red", width=2, dash=(4, 2))
...
def update_stats_and_graph():
    ...
    emissions_before_kg = sum(before_load[h] * CO2_PER_HOUR[h] for h in range(NUM_HOURS))
    emissions_after_kg = sum(after_load[h] * CO2_PER_HOUR[h] for h in range(NUM_HOURS))
    saving_kg_day = emissions_before_kg - emissions_after_kg
    saving_pct_co2 = (
        saving_kg_day / emissions_before_kg * 100
        if emissions_before_kg > 0 else 0.0
    )
