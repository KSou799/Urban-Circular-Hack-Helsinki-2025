...
A 24-hour interactive simulator built for **Urban Circular Hack Helsinki 2025** (U!REKA European University hackathon), answering the challenge:
...
The Helsinki capital region is electrifying fast — heat pumps, EVs, data centres, electric buses, induction kitchens — and the grid is feeling it:
...
Overbuilding the grid is slow, expensive, and carbon-intensive in itself. The smarter, cheaper, faster lever is to **shift flexible loads in time** so the city uses the grid it already has more evenly.
...
**FlexiCity** simulates ~24 flexible loads across a city — EV fleets, heat pumps, office HVAC, supermarket refrigeration, tram and bus depots, data centres, public chargers, schools, hospitals, swimming pools — and compares two scheduling strategies side by side:

| Strategy | Behaviour |
|---|---|
| **Before** *(dumb)* | Every asset starts at the earliest hour of its allowed window — *"plug in when I get home."* |
...
A built-in **simulated-annealing** optimiser (multi-restart, with separate current / best-seen state tracking so it doesn't lose the optimum to accepted-worse moves) finds the slider combo that minimises the peak.
...
The CO₂ profile follows the Nordic grid mix — cleaner midday (solar / hydro), dirtier evening (peak thermal backup) — so shifting load away from the evening spike isn't only cheaper, it's lower-carbon.
...
As the grid's bottleneck moves from *production* to *peak transmission capacity*, flattening peaks is the cheapest carbon-and-cost lever the city has — and it needs no new substations, no new turbines, no new permits.
...
Requires Python 3.x with `tkinter` (bundled with most Python installs). No external dependencies — GUI, graphing, and optimiser are all standard library.
...
- Single shared bus — no distribution-network modelling, no substation-level congestion.
