...
|---|---|---|---|
| Peak demand | 306.0 kW | 276.5 kW | **−9.6 %** |
| Daily cost (flex assets) | €432.57 | €387.21 | **−10.5 %** |
| CO₂ avoided | — | — | **20 kg/day · ~7.3 tCO₂/year** |

*100 % flex participation, balanced 50/50 grid-vs-price priority. The four built-in optimisers can push every one of these numbers further.*
...
There are also four built-in optimisers. Each runs an **exhaustive 1 % × 1 % grid search** over the (flex %, priority %) plane — 10,201 schedules — and reports the slider combination that wins on the chosen objective:

- **Optimize peak** – minimise the worst hour of the day (tiebreaker: cost)
- **Optimize cost** – minimise the daily electricity bill on flex assets (tiebreaker: peak)
- **Optimize CO₂** – minimise daily emissions (tiebreaker: cost)
- **Optimize all** – multi-objective with a **no-regression guardrail**: any schedule where *any* of peak, cost, or CO₂ gets worse than the naive baseline is rejected. No metric is allowed to lose, so the "everyone wins" pitch stays honest.
...
*Hourly Nord Pool spot prices for the Helsinki bidding zone, recorded from the Helen website in November 2025.*
...
3. Hit one of the four optimiser buttons to find the best (flex, priority) combination for that objective. Each runs 10,201 simulations and pops up the result.
...
5. **Reset to defaults** clears any custom assets and snaps the sliders back to 100 % / 50 % so you can start over.

![Optimiser result popup](screenshots/05-optimize-result.png)
...
- Prices are a one-day Nord Pool snapshot recorded from the Helen website in November 2025, not a live feed.
...
- **Live data feeds**: real-time Nord Pool spot prices and Fingrid hourly CO₂ intensity, instead of the static one-day snapshot the simulator ships with.
...
- Hourly Nordic electricity exchange (Nord Pool) prices, recorded from the Helen website in November 2025 (Helen is an electricity distribution company in Helsinki)
